...
st.set_page_config(page_title="Sports Betting AI Dashboard V35 Multi-Sport", layout="wide")

# =========================================================
# CORE MULTI-SPORT CONFIG
# =========================================================
SUPPORTED_SPORTS = {
    "NBA": {
...
# backward-compatible single-state fallbacks
...
def get_selected_sport():
    ...
    return raw if raw in SUPPORTED_SPORTS else DEFAULT_SPORT

def get_sport_config(sport=None):
    sport_name = sport or get_selected_sport()
...
def get_learning_state_for_sport(sport=None):
    sport_name = sport or get_selected_sport()
    sport_states = st.session_state.get("learning_state_by_sport", {})

    if sport_name not in sport_states or not isinstance(sport_states.get(sport_name), dict):
        sport_states[sport_name] = {
            "weights": {
    ...
    st.session_state["learning_state_by_sport"] = sport_states
    return sport_states[sport_name]

def save_learning_state_for_sport(state, sport=None):
    sport_name = sport or get_selected_sport()
    sport_states = st.session_state.get("learning_state_by_sport", {})
    sport_states[sport_name] = state
    st.session_state["learning_state_by_sport"] = sport_states
...
def get_api_mode_for_sport(sport=None):
    ...
    return str(mapping.get(sport_name, "idle")).strip().lower()
...
def get_api_reset_expected_for_sport(sport=None):
    ...
    return str(mapping.get(sport_name, "")).strip()
...
def get_last_pull_epoch_for_sport(sport=None):
    ...
    return float(mapping.get(sport_name, 0) or 0)
...
def get_effective_odds_games(sport=None):
    live_games = get_odds_games_for_sport(sport)
    if live_games:
        return live_games
    return get_cached_games_for_sport(sport)

def get_filtered_bet_log_for_sport(sport=None):
    sport_name = sport or get_selected_sport()
    all_rows = st.session_state.get("bet_log", [])
    filtered = []
    for row in all_rows:
        row_sport = str(row.get("sport", "")).strip().upper()
        if row_sport == sport_name:
            filtered.append(row)
    return filtered
