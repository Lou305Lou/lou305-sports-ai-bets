...
# streamlit_app_v27.py
# V27 Edge Activation Engine + Compact Summary Cards
#
# Focus:
# - compact mobile summary cards
# - edge activation logic to promote strong watchlist plays into Tier C/B
# - heat indicator on cards
# - mobile-first play layout
# - keeps live fetch / fallback / learning / grading / export
...
st.set_page_config(page_title="Sports Betting AI Dashboard V27", layout="wide")
st.title("🔥 Sports Betting AI Dashboard V27")
st.caption("Edge Activation Engine + Compact Summary Cards")

DATA_DIR = Path(".")
BET_LOG_PATH = DATA_DIR / "bet_log_v27.csv"
PROFILE_PATH = DATA_DIR / "learning_profile_v27.csv"
SNAPSHOT_PATH = DATA_DIR / "odds_snapshot_v27.csv"
...
.compact-sub, .summary-sub {
    font-size: 0.92rem;
    opacity: 0.85;
    margin-bottom: 8px;
}
.compact-grid, .summary-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 6px 12px;
    margin-bottom: 8px;
}
.compact-metric, .summary-metric {
    font-size: 0.86rem;
    line-height: 1.15;
}
.compact-label, .summary-label { opacity: 0.72; }
.compact-value, .summary-value { font-weight: 700; }
.compact-reason {
    font-size: 0.83rem;
    opacity: 0.75;
    line-height: 1.2;
...
.tier-a { background: rgba(46,204,113,0.18); }
.tier-b { background: rgba(241,196,15,0.18); }
.tier-c { background: rgba(52,152,219,0.18); }
.tier-watch { background: rgba(149,165,166,0.18); }

.heat-hot { background: rgba(231,76,60,0.16); }
.heat-active { background: rgba(243,156,18,0.16); }
.heat-watch { background: rgba(149,165,166,0.16); }

[data-testid="stMetric"] { padding-top: 0.1rem; padding-bottom: 0.1rem; }
</style>
...
def heat_label(row: pd.Series) -> str:
    ...
    pressure = float(row.get("pressure_score", 0) or 0)
    if edge >= 2.0 and sharp >= 45 and pressure >= 35:
        return "Hot"
    if edge >= 1.0 and sharp >= 38:
        return "Active"
    return "Watch"
...
def compact_card_html(row: pd.Series, idx: int) -> str:
    ...
    heat = heat_label(row)
    heat_class = {"Hot": "heat-hot", "Active": "heat-active", "Watch": "heat-watch"}[heat]
    ...
    <div class="compact-card">
      <div class="compact-title">
        #{idx} <span class="tier-chip {tier_class}">{tier_text}</span><span class="heat-chip {heat_class}">{heat}</span> {title}
      </div>
...
def summary_card_html(title: str, items: List[Tuple[str, str]]) -> str:
    metrics_html = "".join(
        f'<div class="summary-metric"><span class="summary-label">{k}:</span> <span class="summary-value">{v}</span></div>'
        for k, v in items
    )
    return f"""
...
def fetch_odds_from_the_odds_api(api_key: str, sport_key: str, regions: str, markets: str, bookmakers: Optional[str] = None) -> Tuple[pd.DataFrame, str]:
        ...
        for bookmaker in event.get("bookmakers", []):
            ...
            for market in bookmaker.get("markets", []):
                ...
                for outcome in market.get("outcomes", []):
                    rows.append({
                        "event_id": event_id, "sport": sport_key, "game": game, "commence_time": commence_time,
                        "market": mkt_name, "selection": outcome.get("name", ""), "point": outcome.get("point", np.nan),
                        "book": book_title, "odds": outcome.get("price", np.nan),
                    })
...
def attach_previous_snapshot(current_df: pd.DataFrame, snapshot_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.empty:
        ...
        return df
    df["row_key"] = df.apply(lambda r: row_key_from_parts(r["event_id"], r["market"], r["selection"], r["point"], r["book"]), axis=1)
    if snapshot_df.empty:
        df["prev_odds"] = np.nan
        df["line_movement"] = 0.0
        return df.drop(columns=["row_key"], errors="ignore")
    prev = snapshot_df.copy()
    prev["row_key"] = prev.apply(lambda r: row_key_from_parts(r["event_id"], r["market"], r["selection"], r["point"], r["book"]), axis=1)
...
def attach_market_consensus(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if out.empty:
        ...
        return out
    out["group_key"] = out.apply(lambda r: market_group_key_from_parts(r["event_id"], r["market"], r["selection"], r["point"]), axis=1)
    grp = (
        out.groupby("group_key", dropna=False)
        .agg(consensus_price=("odds", "median"), consensus_count=("odds", "size"), min_price=("odds", "min"), max_price=("odds", "max"))
        .reset_index()
    )
...
def update_profile_from_bet_log(bet_log: pd.DataFrame, profile: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = (
        ...
        .agg(bets=("result", "size"), wins=("result", lambda s: (s == "win").sum()), roi_units=("roi_units_single", "sum"))
        .reset_index()
    )
    ...
    for _, r in merged.iterrows():
        bets = int(r["bets"]); wins = int(r["wins"]); roi = float(r["roi_units"])
        if bets < 5:
            w = 1.0
        else:
            wr = wins / bets if bets else 0.5
            roi_per = roi / bets if bets else 0.0
            w = float(np.clip(1.0 + (wr - 0.5) * 1.2 + roi_per * 0.9, 0.70, 1.35))
        weights.append(w)
...
def compute_sharp_score(row: pd.Series) -> float:
    ...
    raw = (
        18
        + normalize_0_100(abs(movement), 0, 35) * 0.42
        + (18.0 if movement > 0 else 0.0)
        + (100 - normalize_0_100(abs(current_odds - consensus_price), 0, 40)) * 0.20
        + normalize_0_100(count, 1, 10) * 0.15
        + normalize_0_100(disagreement, 0, 40) * 0.08
    )
...
def compute_pressure_score(row: pd.Series) -> float:
    ...
    if pd.notna(hours_to_start):
        if hours_to_start <= 3: time_component = 10
        elif hours_to_start <= 8: time_component = 6
        elif hours_to_start <= 24: time_component = 3
    raw = (
        normalize_0_100(abs(movement), 0, 35) * 0.45
        + normalize_0_100(disagreement, 0, 40) * 0.35
        + (15 if best_flag else 0)
        + time_component
    )
    return float(np.clip(raw, 0, 100))
...
def compute_adaptive_thresholds(df: pd.DataFrame, mode: str) -> Dict[str, float]:
    ...
    min_books = 3
    a_score, b_score, c_score = 80.0, 71.0, 63.0
    a_edge, b_edge, c_edge = 3.0, 2.0, 1.25
    a_sharp, b_sharp, c_sharp = 60.0, 48.0, 38.0
    if game_count <= 2:
        a_score -= 2; b_score -= 3; c_score -= 3
        a_edge -= 0.25; b_edge -= 0.25; c_edge -= 0.25
    if avg_disagreement <= 10:
        a_score -= 1; b_score -= 2; c_score -= 2
        a_edge -= 0.25; b_edge -= 0.25; c_edge -= 0.25
    if avg_rows_per_event <= 10:
        min_books = 2
    if mode == "Aggressive":
        a_score -= 3; b_score -= 4; c_score -= 4
        a_edge -= 0.4; b_edge -= 0.4; c_edge -= 0.4
        a_sharp -= 2; b_sharp -= 3; c_sharp -= 4
    elif mode == "Conservative":
        a_score += 2; b_score += 2; c_score += 2
        a_edge += 0.3; b_edge += 0.3; c_edge += 0.3
        a_sharp += 2; b_sharp += 2; c_sharp += 2
    return {
        "min_books": int(np.clip(min_books, 2, 5)),
        "A_score": float(np.clip(a_score, 72, 90)),
        "B_score": float(np.clip(b_score, 64, 82)),
        "C_score": float(np.clip(c_score, 54, 74)),
        "A_edge": float(np.clip(a_edge, 1.8, 5.0)),
        "B_edge": float(np.clip(b_edge, 1.0, 3.5)),
        "C_edge": float(np.clip(c_edge, 0.5, 2.5)),
        "A_sharp": float(np.clip(a_sharp, 52, 80)),
        "B_sharp": float(np.clip(b_sharp, 42, 70)),
        "C_sharp": float(np.clip(c_sharp, 34, 60)),
    }

def edge_activation_override(row: pd.Series, thresholds: Dict[str, float]) -> str:
    edge = float(row.get("edge_pct", 0) or 0)
    sharp = float(row.get("sharp_score", 0) or 0)
    pressure = float(row.get("pressure_score", 0) or 0)
    cc = float(row.get("consensus_count", 0) or 0)
    disagreement = float(row.get("price_disagreement", 0) or 0)
    best_flag = bool(row.get("best_price_flag", False))
    move = float(row.get("line_movement", 0) or 0)

    # Soft activation to Tier C
    if edge >= 1.25 and sharp >= 38 and cc >= max(3, thresholds["min_books"]):
        return "C"

    # Best price + edge + movement boost
    if best_flag and edge >= 1.0 and move > 0 and pressure >= 35:
        return "C"

    # Market disagreement override
    if disagreement >= 10 and edge >= 1.5 and cc >= max(3, thresholds["min_books"]):
        return "B"
...
def assign_tier(row: pd.Series, thresholds: Dict[str, float]) -> str:
    ...
    if cc >= thresholds["min_books"] and fs >= thresholds["A_score"] and ep >= thresholds["A_edge"] and ss >= thresholds["A_sharp"]:
        tier = "A"
    elif cc >= thresholds["min_books"] and fs >= thresholds["B_score"] and ep >= thresholds["B_edge"] and ss >= thresholds["B_sharp"]:
        tier = "B"
    elif cc >= thresholds["min_books"] and fs >= thresholds["C_score"] and ep >= thresholds["C_edge"] and ss >= thresholds["C_sharp"]:
        tier = "C"
    else:
        tier = ""

    override_tier = edge_activation_override(row, thresholds)
    priority = {"": 0, "C": 1, "B": 2, "A": 3}
    return override_tier if priority[override_tier] > priority[tier] else tier

def score_live_candidates(df: pd.DataFrame, bankroll: float, max_units: float) -> pd.DataFrame:
    out = df.copy()
    sharp_scores=[]; ineff_scores=[]; edge_pcts=[]; pressure_scores=[]; final_scores=[]; units=[]; reasons=[]
    for _, r in out.iterrows():
        ...
        disagreement = float(r.get("price_disagreement", 0.0))
        ...
        if edge_pct >= 1.25 and sharp >= 38 and consensus_count >= 3:
            activation_boost += 6.0
        if best_flag and edge_pct > 0 and movement > 0:
            activation_boost += 4.0
        if disagreement >= 10 and edge_pct > 0:
            activation_boost += 5.0
        ...
        recommended_units = float(np.clip(bankroll * kf * 0.25 / 100.0, 0.0, max_units))

        rb=[]
        if best_flag: rb.append("best market price")
        if consensus_count >= 4: rb.append(f"{consensus_count}/book agreement")
        if sharp >= 70: rb.append("strong movement")
        if pressure >= 60: rb.append("market pressure")
        if edge_pct >= 1.25: rb.append(f"+{edge_pct:.1f}% edge")
        if disagreement >= 10: rb.append("book disagreement")
        if not rb: rb.append("watch only")

        sharp_scores.append(round(sharp,1)); ineff_scores.append(round(ineff,1)); edge_pcts.append(round(edge_pct,2))
        pressure_scores.append(round(pressure,1)); final_scores.append(round(final_score,1)); units.append(round(recommended_units,2))
...
# Session state
if "bet_log_v27" not in st.session_state:
    st.session_state.bet_log_v27 = safe_read_csv(BET_LOG_PATH, default_bet_log())
if "profile_v27" not in st.session_state:
    st.session_state.profile_v27 = safe_read_csv(PROFILE_PATH, default_profile())
if "snapshot_v27" not in st.session_state:
    st.session_state.snapshot_v27 = safe_read_csv(SNAPSHOT_PATH, default_snapshot())
if "current_live_df_v27" not in st.session_state:
    st.session_state.current_live_df_v27 = fallback_live_rows()

bet_log = st.session_state.bet_log_v27.copy()
profile = st.session_state.profile_v27.copy()
snapshot_df = st.session_state.snapshot_v27.copy()
...
mode = st.sidebar.selectbox("Activation Mode", ["Aggressive", "Balanced", "Conservative"], index=0)
...
if reset_snapshot:
    ...
    st.session_state.snapshot_v27 = snapshot_df.copy()
...
if refresh_learning:
    ...
    st.session_state.profile_v27 = profile.copy()
...
if fetch_live or auto_refresh:
    if api_key.strip():
        try:
            live_df, requests_remaining = fetch_odds_from_the_odds_api(api_key.strip(), sport_key, regions.strip(), ",".join(markets_list), bookmakers.strip() or None)
            if live_df.empty:
                live_status = "Live fetch returned no rows. Fallback data loaded."
                live_df = fallback_live_rows()
            else:
                live_status = "Live odds fetched successfully."
            st.session_state.current_live_df_v27 = live_df.copy()
...
current_live_df = st.session_state.current_live_df_v27.copy()
...
scored_df = scored_df.sort_values(["auto_qualified", "tier_sort", "final_score", "edge_pct"], ascending=[False, True, False, False]).reset_index(drop=True)
...
avg_edge = f"{qualified_df['edge_pct'].mean():.2f}%" if len(qualified_df) else "—"
summary1 = summary_card_html("Market Snapshot", [
    ("Live Rows", str(len(scored_df))),
    ("Qualified", str(len(qualified_df))),
    ("Watchlist", str(len(watch_df))),
    ("Mode", mode),
])
summary2 = summary_card_html("Tier Summary", [
    ("Tier A", str(int((qualified_df["tier"] == "A").sum()) if len(qualified_df) else 0)),
    ("Tier B", str(int((qualified_df["tier"] == "B").sum()) if len(qualified_df) else 0)),
    ("Tier C", str(int((qualified_df["tier"] == "C").sum()) if len(qualified_df) else 0)),
    ("Avg Edge", avg_edge),
])
...
if qualified_df.empty:
    st.warning("No qualified plays right now.")
else:
...
quick = scored_df.copy()
quick["tier"] = quick["tier"].replace("", "Watch")
st.dataframe(
    quick[["tier", "game", "selection", "book", "odds", "recommended_units", "final_score", "edge_pct"]],
...
if auto_save and not qualified_df.empty:
    ...
    for _, r in qualified_df.iterrows():
        key = " | ".join([str(r["game"]), str(r["market"]), str(r["selection"]), str(r["point"]), str(r["book"]), str(r["odds"])])
        if key not in existing_keys:
            add_rows.append({
                "date": pd.Timestamp.now().strftime("%Y-%m-%d"),
                "sport": r["sport"], "game": r["game"], "market": r["market"], "selection": r["selection"],
                "point": r["point"], "book": r["book"], "odds": r["odds"], "prev_odds": r["prev_odds"],
                "consensus_price": r["consensus_price"], "consensus_count": r["consensus_count"], "line_movement": r["line_movement"],
                "sharp_score": r["sharp_score"], "inefficiency_score": r["inefficiency_score"], "edge_pct": r["edge_pct"],
                "pressure_score": r["pressure_score"], "final_score": r["final_score"], "tier": r["tier"],
                "recommended_units": r["recommended_units"], "result": "", "closing_odds": np.nan, "clv": np.nan,
            })
    if add_rows:
        bet_log = pd.concat([bet_log, pd.DataFrame(add_rows)], ignore_index=True)
        st.session_state.bet_log_v27 = bet_log.copy()
...
with st.form("bet_log_form"):
    ...
    if submitted:
        ...
        temp_row = pd.Series({
            "odds": odds, "prev_odds": prev_odds, "consensus_price": consensus_price, "consensus_count": consensus_count,
            "price_disagreement": abs(consensus_price - odds), "best_price_flag": True, "line_movement": line_movement, "hours_to_start": np.nan,
        })
        ...
        consensus_bonus = {5: 14, 4: 9, 3: 4, 2: 1}.get(consensus_count, -6)
        activation_boost = 0.0
        if edge_pct >= 1.25 and sharp_score >= 38 and consensus_count >= 3:
            activation_boost += 6.0
        final_score = float(np.clip(
            sharp_score * 0.32 + ineff * 0.24 + pressure_score * 0.18 + learning_weight * 16.0 +
            consensus_bonus + 8 + min(abs(line_movement) * 0.5, 8) + activation_boost,
            0, 100
        ))
        ...
        temp_scored = pd.Series({
            "consensus_count": consensus_count, "final_score": final_score, "edge_pct": edge_pct, "sharp_score": sharp_score,
            "pressure_score": pressure_score, "price_disagreement": abs(consensus_price - odds), "best_price_flag": True, "line_movement": line_movement
        })
        tier = assign_tier(temp_scored, thresholds)
        clv = clv_from_odds(odds, closing_odds)
        add = pd.DataFrame([{
            "date": pd.Timestamp.now().strftime("%Y-%m-%d"), "sport": sport_key, "game": game, "market": market, "selection": selection,
            "point": np.nan if point == 0 else point, "book": book, "odds": odds, "prev_odds": prev_odds, "consensus_price": consensus_price,
            "consensus_count": consensus_count, "line_movement": line_movement, "sharp_score": round(sharp_score, 1),
            "inefficiency_score": round(ineff, 1), "edge_pct": round(edge_pct, 2), "pressure_score": round(pressure_score, 1),
            "final_score": round(final_score, 1), "tier": tier, "recommended_units": round(units, 2), "result": result,
            "closing_odds": closing_odds, "clv": round(clv, 2),
        }])
...
with e1:
    st.download_button("Download Bet Log CSV", data=bet_log.to_csv(index=False).encode("utf-8"), file_name="bet_log_v27.csv", mime="text/csv", use_container_width=True)
with e2:
    st.download_button("Download Learning Profile CSV", data=profile.to_csv(index=False).encode("utf-8"), file_name="learning_profile_v27.csv", mime="text/csv", use_container_width=True)
with e3:
    st.download_button("Download Snapshot CSV", data=snapshot_df.to_csv(index=False).encode("utf-8"), file_name="odds_snapshot_v27.csv", mime="text/csv", use_container_width=True)

st.caption("V27 adds edge activation promotion, compact summary cards, and heat tags so strong live signals turn into actionable plays more often.")
